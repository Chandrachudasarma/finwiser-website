...
Five concrete workloads in the production codebase are GPU- and cloud-relevant. This note describes each, its current implementation, the kind of managed component it maps to, and the existing benchmark/harness that makes a comparison defensible.
...
**Why GPU / cloud fit:** Financial PII must remain on Indian infrastructure. The privacy gateway, breakers, streaming machinery, and structured-output handling are already in production — the only swap point is the LLM endpoint itself, a `baseURL` change in one file (`utils/openai-client.js:39`). A hosted in-region LLM endpoint (e.g. Gemini on Vertex AI in `asia-south1`, or a self-hosted Llama-3.1-8B-Instruct on managed GPU) is a drop-in replacement for `gpt-4o-mini`-class calls.

**Why this is real, not aspiration:** An offline eval harness (`scripts/eval-prompts.js`) with a 5-axis rubric, LLM-as-judge scoring, and golden-prompt fixtures already exists — we can replay every prompt through any new endpoint and measure parity directly.

**The sovereign-inference angle:** Today the privacy gateway *must* strip identifiers and withhold amounts because data crosses to the US. With in-region inference, that stripping becomes defence-in-depth rather than a precondition — the advisor can be given richer context and answers improve.
...
**Why GPU / cloud fit:** P2B is the accuracy ceiling on novel merchants but is currently bottlenecked by host RAM, not methodology. **GPU-served NLI on a managed inference platform** (e.g. Vertex AI Prediction with a TensorRT- or ONNX-Runtime-optimised model) brings BART-large-MNLI from ~8 s to tens of ms, eliminates the silent fallback to the less-accurate model, and removes the memory guard. Dynamic batching fits our batch-oriented evaluation path.
...
**Why GPU / cloud fit:** This is the strongest GPU candidate in the entire product. Same math over ~14K independent NAV series — embarrassingly parallel, columnar, batchable.

- **Columnar analytics on a managed platform** (e.g. BigQuery + Dataproc, with RAPIDS for GPU vectorisation): log returns, vol, beta, rolling windows, and `groupby().quantile([.25, .5, .75])` for distribution stats become single vectorised calls.
- **Batched GPU linear algebra** (cuPy/cuML on a managed GPU cluster): stack all funds into a 3-D tensor and solve every fund's system in one batched linear solve. ~20,000 tiny inversions is exactly where batched GPU linear algebra wins.
...
**Why GPU / cloud fit:** I.i.d. paths, embarrassingly parallel. GPU parallelism (on a managed GPU instance or batch-compute service) runs 10⁵–10⁶ paths per goal in milliseconds, replacing the linear heuristic with a real CDF over outcome distributions. Per-user latency budget allows it comfortably.
...
**Why GPU / cloud fit:** A small instruction-tuned LLM (7–8B class — e.g. Gemini Flash, Llama-3.1-8B, served on a managed in-region endpoint) expands a code plus its numeric context into one personalised Hindi/English sentence. **Crucially, the codes remain the source of truth and audit trail — the LLM only renders, never decides.** This preserves SEBI explainability (a learned model on the *decision* would be a compliance defect) while materially improving UX.

**Why this is real:** The 8 codes are already enumerated in production code; the calculation audit log captures `{params, results, reconciliation}` per decision with a 90-day TTL. A small-LLM renderer plugs in at the presentation layer without touching the decision engine or the audit chain.
...
## Proposed benchmark sprint (60-day GPU benchmark experiment)
...
1. **Self-hosted in-region LLM vs `gpt-4o-mini`** — Llama-3.1-8B (or Gemini) behind the same `baseURL`, replay golden-prompt fixtures through the 5-axis eval harness. Apples-to-apples.
2. **GPU-served NLI on a managed inference platform** — replay 21 golden profile fixtures + AA replay harness; measure removal of the `layer2Skipped` degradation.
3. **Columnar batch analytics** — replay one full peer-ranking job over the ~14K-scheme NAV table; target 10–30 min → low-single-digit minutes; golden-file parity test (±0.5% vs Value Research / Morningstar reference values).
...
- **Production codebase:** ~50k LOC Node backend (`Finwiser_backend/`) + Flutter cross-platform frontend; MVP in closed testing
